...
test("project page distinguishes stable, previous, and preview evidence", async ({ page, request }) => {
  ...
  const { index, policy } = await test.step("release data endpoints", async () => {
    const indexResponse = await request.get("/data/release-history.json");
    expect(indexResponse.ok()).toBe(true);
    const releaseIndex = await indexResponse.json() as {
    ...
    expect(releaseIndex.releases.map((release) => release.channel)).toEqual(["stable", "previous", "preview"]);
    expect(new Set(Object.values(releaseIndex.channels)).size).toBe(3);
    ...
    expect(policyData.schemaVersion).toBe(1);
    expect(policyData.candidate.eligible).toBe(policyData.decision === "promote");
    return { index: releaseIndex, policy: policyData };
  });

  await test.step("security headers", async () => {
  ...
  await test.step("document metadata and social preview", async () => {
  ...
  await test.step("release status table", async () => {
  ...
  await test.step("permission-prompt demo", async () => {
  ...
  await test.step("runtime compatibility reports", async () => {
  ...
  await test.step("downloads and community links", async () => {
  ...
  await test.step("dependency diff vs stable", async () => {
  ...
  await test.step("gateway contract diff vs stable", async () => {
  ...
  await test.step("promotion policy card", async () => {
...
test("release ledger remains readable at a mobile viewport", async ({ page }) => {
  ...
  await test.step("release ledger fits the viewport", async () => {
  ...
  await test.step("expanded gateway diff fits the viewport", async () => {
  ...
  await test.step("promotion policy fits the viewport", async () => {
  ...
  await test.step("broker section fits the viewport", async () => {
