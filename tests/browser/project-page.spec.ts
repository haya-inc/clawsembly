...
test("project page distinguishes stable, previous, and preview evidence", async ({ page, request }) => {
  ...
  await expect(page.getByText("challenge · Ed25519 · hello-ok", { exact: true })).toBeVisible();
  ...
  await expect(runtimes.getByText("Handshake contract ✓", { exact: true })).toBeVisible();
  await expect(runtimes.getByText(/pins the browser origin/)).toBeVisible();
  await expect(runtimes.getByText(/artifact-matched protocol 4 challenge/)).toBeVisible();
  await expect(runtimes.getByText(/digest-pinned Node client carries typed broker calls/)).toBeVisible();
