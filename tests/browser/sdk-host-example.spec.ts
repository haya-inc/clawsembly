...
const report = JSON.parse(readFileSync(resolve("apps/web/public/data/compatibility.json"), "utf8")) as {
...
test("packed SDK host renders the current fail-closed launch decision", async ({ page }) => {
  ...
  await page.route(reportURL, async (route) => {
    reportRequests += 1;
    await route.fulfill({ json: report, headers: { "Access-Control-Allow-Origin": "*" } });
  });
