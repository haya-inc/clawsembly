...
assert.match(npmPublish, /for attempt in \$\(seq 1 30\)/u, "npm publishing must tolerate registry propagation before integrity verification");
assert.match(npmPublish, /npm publish .* \|\| publish_status=\$\?/u, "npm publishing must judge success by the final registry state, not the publish exit code alone");
